...
function renderCallRow(c, isAdmin = false) {
  ...
  let statusIconHtml = '';
  if (!hasReport) {
    statusIconHtml = `<div class="status-icon-cell missing" title="Brak raportu">!</div>`;
  } else {
    const iconClass = { 'NOWY_PACJENT': 'new', 'STALY_PACJENT': 'regular', 'WIZYTA_BIEZACA': 'visit', 'SPAM': 'spam' }[c.contactType] || 'regular';
    const iconLabel = { 'NOWY_PACJENT': 'N', 'STALY_PACJENT': 'S', 'WIZYTA_BIEZACA': 'W', 'SPAM': 'X' }[c.contactType] || '?';
    statusIconHtml = `<div class="status-icon-cell ${iconClass}" title="${c.contactType}">${iconLabel}</div>`;
  }
        ...
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <div class="call-name-cell">
            <div style="display:flex; align-items:center; gap:8px;">
              <div class="call-name" onclick="event.stopPropagation(); openPatientCard('${c.contactId}', '${escHtml(c.contactName || '')}')" style="color:#3b82f6; cursor:pointer; font-weight:700;">${escHtml(c.contactName || (c.from !== '0' ? c.from : '') || (c.to !== '0' ? c.to : '') || 'Nieznany')}</div>
              ${renderPatientStatusBadge(c.patientStatus)}
            </div>
            <div class="call-number" onclick="event.stopPropagation(); openPatientCard('${c.contactId}', '${escHtml(c.contactName || '')}')" style="color:#3b82f6; cursor:pointer;">${escHtml((c.from !== '0' ? c.from : '') || (c.to !== '0' ? c.to : '') || '')}</div>
...
async function fetchPopupEnrichment(contactId) {
  try {
    ...
    if (data.zglosza) {
      const zgloszaVal = document.getElementById('popupZglosza');
      if (zgloszaVal) zgloszaVal.textContent = data.zglosza;
      const zgloszaInfo = document.getElementById('popupZCzymSieZglasza');
      if (zgloszaInfo) zgloszaInfo.classList.remove('hidden');
    }
    ...
    if (data.source || data.leadSource) {
      ...
      const sourceInputLegacy = document.getElementById('zrodloLeada');
      if (sourceInputLegacy) sourceInputLegacy.value = data.source || data.leadSource;
    }
...
// AKCJE AUTOMATYCZNE (Punkt: Akcje automatyczne)
...
function closeResignationModal() {
  document.getElementById('resignationModal').classList.add('hidden');
...
async function saveQuickAction(tag, note = '') {
  try {
    ...
    if (!contactId || contactId === 'unknown') {
      showToast('Błąd: Nieznany kontakt', 'error');
      return;
    }
    
    // 1. Dodaj tag w GHL
    await fetch(`/api/contact/${contactId}/tags`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tags: [tag] })
    });

    // 2. Zapisz notatkę jeśli podano
    if (note) {
      await fetch(`/api/contact/${contactId}/notes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ body: `Powód: ${note}`, userId: currentUser?.id })
      });
    }

    // 3. Zapisz raport jako SPAM/Niekwalif
    if (activeCallId) {
...
function openCreateTaskModalFromCall() {
  if (typeof openCreateTaskModal === 'function') {
    openCreateTaskModal(currentContact?.id, currentContact?.name);
  } else {
    showToast('Funkcja tworzenia zadań jest niedostępna', 'error');
  }
}
...
async function saveReport(forceClose = true) {
  if (!selectedStatus) { 
    showToast('Wybierz status pacjenta', 'error'); 
    const statusHeader = document.querySelector('.popup-report h3');
    if (statusHeader) {
      statusHeader.style.color = '#ef4444';
      statusHeader.textContent = '⚠ Wybierz status pacjenta (wymagane)';
      setTimeout(() => {
        statusHeader.style.color = '';
        statusHeader.textContent = 'Raport z rozmowy';
      }, 3000);
    }
    return; 
  }
...
function renderPatientCardInfo(data) {
  ...
  // Renderowanie tagów w karcie pacjenta
  ...
  if (tagsContainer) {
    if (contact.tags && Array.isArray(contact.tags) && contact.tags.length > 0) {
      tagsContainer.innerHTML = contact.tags.map(t => `<span style="font-size:10px; background:#e2e8f0; padding:2px 8px; border-radius:10px;">${t}</span>`).join('');
    } else {
      tagsContainer.textContent = '—';
    }
  }

  // Wyświetlanie ogólnego statusu pacjenta w karcie (Punkt: Ogólny status)
  const statusHeader = document.getElementById('patientCardStatusBadge');
  if (statusHeader) {
    const possibleStatuses = ['czeka_na_kontakt', 'nie_odbiera_w_procesie', 'nie_odbiera_przegrana', 'prosi_o_kontakt', 'umowiony_na_w0', 'niekwalifikowany', 'rezygnacja'];
    const foundStatus = contact.patientStatus || contact.tags?.find(t => possibleStatuses.includes(t));
    statusHeader.innerHTML = foundStatus ? renderPatientStatusBadge(foundStatus) : '';
  }
