...
function handleCallRinging(data) {
  ...
  // Admin NIE dostaje popup — tylko obserwuje statystyki
  // Recepcja: zawsze; Opiekun: tylko swoje połączenia
  const isMyCall =
    myRole === 'reception' ||
    (myRole === 'opiekun' && data.userId === myUserId);
...
function resetReportForm() {
  ...
  // Reset statusBar warstwy 1
  const statusBar = document.getElementById('popupSelectedStatusBar');
  if (statusBar) statusBar.style.display = 'none';
  document.querySelectorAll('.popup-status-tile').forEach(b => b.classList.remove('active'));
...
function selectStatus(status) {
  ...
  // Zaznacz właściwy kafelek w warstwie 1
  document.querySelectorAll('.popup-status-tile').forEach(b => b.classList.remove('active'));
  const activeTile = document.querySelector(`[data-status="${status}"].popup-status-tile`);
  if (activeTile) activeTile.classList.add('active');

  // Pokaż pasek potwierdzenia
  const statusBar = document.getElementById('popupSelectedStatusBar');
  const statusLabel = document.getElementById('popupSelectedStatusLabel');
  const statusNames = { NOWY_PACJENT: '🆕 Nowy pacjent', STALY_PACJENT: '🔄 Stały pacjent', WIZYTA_BIEZACA: '📅 Wizyta bieżąca', SPAM: '🚫 Nie pacjent' };
  if (statusBar) statusBar.style.display = 'block';
  if (statusLabel) statusLabel.textContent = statusNames[status] || status;

  // Przygotuj formularz (ukryty, przeniesiony do panelu)
  document.querySelectorAll('.status-btn, .status-tile').forEach(b => b.classList.remove('active'));
  const btn = document.querySelector(`[data-status="${status}"]:not(.popup-status-tile)`);
  if (btn) btn.classList.add('active');
  document.querySelectorAll('.report-form').forEach(f => f.classList.add('hidden'));
...
async function saveReport(forceClose = true) {
  ...
  try {
    ...
    // ── Natychmiast zaktualizuj lokalny store allCalls żeby tabela pokazała "✓ Uzupełniony" ──
    if (activeCallId) {
      const callIdx = allCalls.findIndex(c => c.callId === activeCallId);
      if (callIdx >= 0) {
        allCalls[callIdx].contactType = selectedStatus;
        allCalls[callIdx].callEffect  = reportData.outcome || selectedOutcome || 'connected';
