...
function resetReportForm() {
  ...
  // Wyczyść wszystkie pola formularza raportu
	  ['programLeczenia', 'dataW0', 'contactDateTime', 'powodRezygnacji',
	   ...
	   'notatkaNowy', 'notatkaStaly', 'notatkaWizyta'].forEach(id => {
    const el = document.getElementById(id);
  ...
  try {
    ...
    if (activeCallId) {
      ...
      try {
        ...
        // Notatka z konkretnych formularzy
    ...
    // Logika zamykania zależna od miejsca kliknięcia
    closeReportFormPanel();
    if (forceClose) {
      closeCallPopup(true); // Zamknij wszystko
    }
