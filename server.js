...
/**
 * AUTOMATYCZNA LOGIKA STATUSÓW (Reguły czasowe/ilościowe)
 * 🔵 nie_odbiera_w_procesie — po nieodebranym, < 5 dni i < 2 próby
 * ⚫ nie_odbiera_przegrana — po 5 dniach i 2+ próbach
 */
async function autoUpdatePatientStatus(contactId, phone) {
  if (!contactId || !GHL_TOKEN) return;
  try {
    // 1. Pobierz dane kontaktu z GHL (ilość prób, data stworzenia)
    const resp = await axios.get(`https://services.leadconnectorhq.com/contacts/${contactId}`, { headers: ghlHeaders });
    ...
    const attempts = parseInt(getFieldByKey('contact_attempts')?.value || '0') || 0;
    const createdAt = new Date(contact.dateAdded);
    const daysSinceCreated = (Date.now() - createdAt.getTime()) / (1000 * 60 * 60 * 24);
    
    let newStatus = null;
    
    // Reguła: Przegrana (5+ dni i 2+ próby)
    if (daysSinceCreated >= 5 && attempts >= 2) {
      newStatus = PATIENT_STATUS_TAGS.NIE_ODBIERA_PRZEGRANA.key;
    } 
    // Reguła: W procesie (nieodebrane, ale jeszcze nie przegrana)
    else {
      newStatus = PATIENT_STATUS_TAGS.NIE_ODBIERA_W_PROCESIE.key;
    }
    
    if (newStatus) {
      await setPatientStatusTag(contactId, newStatus);
    }
  } catch (e) {
...
async function autoMoveStageForMissedCall(phone) {
  ...
  try {
    ...
    if (nextStage) {
      ...
      // AUTOMATYCZNY STATUS (Punkt: nie_odbiera_w_procesie / przegrana)
      autoUpdatePatientStatus(contact.id, phone).catch(() => {});
    }
  } catch(e) {
