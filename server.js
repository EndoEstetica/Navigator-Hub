...
const OPPS_CACHE_TTL = 90 * 1000; // 90 sekund (było 60s — redukuje liczbę requestów do GHL)
...
app.get('/api/contacts/new', async (req, res) => {
  ...
  try {
    ...
    cache.set(cacheKey, result, 90 * 1000); // 90s cache (było 60s)
    res.json(result);
  } catch (err) {
...
app.get('/api/contact/:id/popup', async (req, res) => {
  ...
  if (cached && (Date.now() - cached.ts < 60000)) { // 60s cache (było 30s)
...
app.get('/api/contact/:id/card', async (req, res) => {
  try {
    ...
    if (cached && (Date.now() - cached.ts < 120000)) { // 2 min cache (było 60s)
